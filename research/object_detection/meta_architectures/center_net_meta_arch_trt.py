...
def tensor_scatter_nd_add(tensor, indices, updates):
    print("PATCHED SCATTER ND ADD")
    scatter_nd = tf.scatter_nd(indices, updates, tensor.shape)
    return tf.math.add(scatter_nd, tensor)
...
def top_k_feature_map_locations(
    ...
    if per_channel:  # not taken
        if k == 1:
            ...
            safe_k = k
            # assert k < tf.shape(feature_map_peaks_transposed)[-1]
...
class CenterNetMetaArchTrt(model.DetectionModelPrediction):
    ...
    def __init__(
        ...
        super(CenterNetMetaArchTrt, self).__init__(num_classes)
    ...
    def _postprocess_keypoints_multi_class(
        ...
        for ex_ind in range(batch_size):
            ...
            for task_name, kp_params in self._kp_params_dict.items():
                ...
                instance_mask = self._get_instance_indices(
                    classes, num_detections, ex_ind, kp_params.class_id
                )
                instance_inds = tf.range(100)
                ...
                instance_mask = tf.repeat(
                    instance_mask, len(kp_params.keypoint_indices), axis=1
                )
                instance_mask = tf.reshape(instance_mask, [-1])
                instance_mask = tf.expand_dims(instance_mask, axis=1)
                instance_mask = tf.repeat(instance_mask, 2, axis=1)
                ...
                kpt_coords_for_class = tf.where(
                    instance_mask,
                    kpt_coords_for_class,
                    tf.zeros_like(kpt_coords_for_class),
                )
                kpt_scores_for_class = tf.where(
                    instance_mask[:, 0],
                    kpt_scores_for_class,
                    tf.zeros_like(kpt_scores_for_class),
                )

                kpt_coords_for_example_all_det = tensor_scatter_nd_add(
                    kpt_coords_for_example_all_det, combined_inds, kpt_coords_for_class
                )
                kpt_scores_for_example_all_det = tensor_scatter_nd_add(
    ...
    def _get_instance_indices(self, classes, num_detections, batch_index, class_id):
        ...
        return valid_detections_with_kpt_class
    ...
    def restore_from_objects(self, fine_tune_checkpoint_type="detection"):
        ...
        if fine_tune_checkpoint_type == "detection":
            feature_extractor_model = tf.train.Checkpoint(
                ...
                _prediction_head_dict=self._prediction_head_dict,
            )
